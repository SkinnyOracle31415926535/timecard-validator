...
const TRANSFER_SOURCE = readFileSync(
...
function load(values) {
  ...
  vm.runInContext(TRANSFER_SOURCE, context);
...
test("temporary transfer client remains browser-local", () => {
  assert.equal(TRANSFER_SOURCE.includes("/api/app-sync"), false);
  assert.equal(/\bfetch\s*\(/.test(TRANSFER_SOURCE), false);
  assert.equal(/private\s+(?:device\s+)?sync|semantic/i.test(TRANSFER_SOURCE), false);
  assert.equal(Object.hasOwn(load().api, "buildConflictReviewBundle"), false);
});
